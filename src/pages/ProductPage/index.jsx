...
import { setFavoriteItem } from '../../redux/slices/favoriteSlice';
...
const Product = () => {
	...
	const [favorite, setFavorite] = useState(useSelector((state) => state.favorite.items.find(item => item.id === id)));

	useEffect(() => {
		async function fetchProduct(){
			try {
				const { data } = await axios.get('https://621510a4cdb9d09717ac4a98.mockapi.io/guitars/' + id);
				setProduct(data);
	...
	// const { id, imageUrl, name, price, description } = JSON.parse(localStorage.getItem('activeItem'));
	...
	const addProductToFavorite = () => {
		...
		dispatch(setFavoriteItem({
			...
			imageUrl: product.imageUrl,
			price: product.price,
			description: product.description ? product.description : 'Описание отсутствует',
		}))
	}
		...
		<div className={styles.productItem}>
			...
			<div className={styles.product}>
				...
				<div className={styles.info}>
					{product.description ? <div className={styles.description}>{product.description}</div> : 'нет описания'}

					<table className={styles.table}>
						<thead></thead>
						<tbody>
							<tr>
								<td className={styles.tableKey}>Наличие</td>
								<td>В наличии в нашем магазине  или отправляем курьерской службой СДЭК или ЕМС</td>
							</tr>
							<tr>
								<td className={styles.tableKey}>Возврат</td>
								<td>30 дней</td>
							</tr>
							<tr>
								<td className={styles.tableKey}>Гарантия</td>
								<td>Пожизненная</td>
							</tr>
						</tbody>
					</table>
					<div className={styles.price}>Цена:{product.price.toLocaleString()} ₽</div>
					<div className={styles.buttons}>
						<Button onClick={addProductToCart}/>
						<div  className={cn(styles.favorites, {[styles.favoritesAct]: favorite})}
